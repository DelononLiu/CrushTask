      ...
      <div className="flex-1 overflow-y-auto p-3 lg:p-4 space-y-2 lg:space-y-3">
        {messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] lg:max-w-[85%] p-2 lg:p-3 rounded-xl text-sm ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-200'}`}>
      ...
      <div className="p-2 border-t border-gray-800">
        <div className="flex gap-2 items-center">
          ...
          <input 
            ...
            placeholder="输入..." 
            className="flex-1 bg-gray-800 text-white px-2 lg:px-3 py-1 lg:py-1.5 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" 
          />
          <button onClick={onSend} className="px-2 lg:px-3 py-1 lg:py-1.5 bg-blue-600 text-white rounded text-xs lg:text-sm">发送</button>
        </div>
      </div>
    </div>
  );
