    ...
    <div className="p-2 lg:p-3 border-b border-gray-800 bg-gradient-to-r from-gray-900 to-gray-800">
      ...
      <div className="flex items-end justify-between">
        <div className="flex items-end gap-1">
          <h1 className="text-base lg:text-xl font-semibold text-white leading-none">{task.title}</h1>
          <span className={`px-0.5 py-0.25 text-[8px] leading-none font-medium border ${statusColors[task.status]}`}>{statusLabels[task.status]}</span>
          <span className="px-0.5 py-0.25 text-[8px] leading-none font-medium bg-gray-500/20 text-gray-400 border border-gray-500/50">{priorityLabels[task.priority]}</span>
        </div>
        <div className="flex items-end gap-1">
          {onEdit && (
